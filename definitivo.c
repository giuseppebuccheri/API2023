...
void aggiungi_auto(struct stazione** head, int distanza, int autonomia) {
    ...
    //Scorro finche non trovo la stazione a cui aggiungere l'auto
    while (curr != NULL) {
        if (curr->chilometro == distanza) {
            struct automobile *nuova_auto = malloc(sizeof(struct automobile));
            nuova_auto->autonomia = autonomia;
            curr->n_auto++;
            nuova_auto->next = curr->automobili;
            curr->automobili = nuova_auto;
            if(nuova_auto->autonomia > curr->max_auto)
                curr->max_auto = nuova_auto->autonomia;
            printf("aggiunta\n");
            return;
        }
        curr = curr->prev;
    ...
    while (curr != NULL) {
        if (curr->chilometro == distanza) {
            if (curr->n_auto == 0) {
                ...
                struct automobile* testa = curr->automobili;
                if(testa->autonomia == autonomia){
                    curr->automobili = curr->automobili->next;
                    curr->max_auto = calcolo_max_auto(&(curr->automobili));
                    free(testa);
                    printf("rottamata\n");
                    return;
                }
